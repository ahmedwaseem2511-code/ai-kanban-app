...
export default function TaskModal({ task, onClose }: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (task) {
      const { data } = await supabase.from('tasks').update(taskData).eq('id', task.id).select().single()
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-slate-800 rounded-2xl p-6 w-[460px] border border-slate-700" onClick={e => e.stopPropagation()}>
        <h2 className="text-lg font-bold text-white mb-5">{task ? 'Edit Task' : 'Add New Task'}</h2>
        {error && <p className="text-red-400 text-sm mb-3 bg-red-900/20 p-2 rounded-lg">{error}</p>}
        <div className="mb-4">
          <label className="text-slate-400 text-sm mb-1 block">Title</label>
          <input value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-slate-900 border border-slate-600 text-white p-3 rounded-lg text-sm outline-none" placeholder="Task title..." />
        </div>
        <div className="mb-4">
          <label className="text-slate-400 text-sm mb-1 block">Description</label>
          <textarea value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-slate-900 border border-slate-600 text-white p-3 rounded-lg text-sm outline-none h-20 resize-none" placeholder="Description..." />
          <button onClick={handleAIGenerate} disabled={aiLoading} className="mt-2 bg-indigo-900/50 text-indigo-300 border border-indigo-700 px-3 py-2 rounded-lg text-sm">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading} className="flex-1 bg-indigo-600 text-white py-3 rounded-lg font-semibold">
